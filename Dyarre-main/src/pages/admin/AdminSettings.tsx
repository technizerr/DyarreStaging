...
interface TelegramConfig { botUsername: string; allowedChatIds: string; welcomeMessage: string; defaultStatus: string; defaultFurnishing: string; defaultCompletion: string; autoPublish: boolean }
...
export default function AdminSettings() {
  ...
  useEffect(() => {
    const keys = ['backend_config', 'design_config', 'meta_pixel', 'tracking_config', 'whatsapp_business', 'mcp_config', 'ai_config', 'social_media', 'telegram_bot']
    Promise.all(keys.map(k => getSetting(k).then(v => ({ key: k, value: v })))).then(results => {
      results.forEach(({ key, value: v }) => {
        if (!v || typeof v !== 'object') return
        const obj = v as Record<string, unknown>
        if (key === 'backend_config') setBackend({ ...defaultBackend, ...obj as Partial<BackendConfig> })
        if (key === 'design_config') setDesign({ ...defaultDesign, ...obj as Partial<DesignConfig> })
        if (key === 'meta_pixel') setPixel({ metaPixelId: '', ...obj as Partial<PixelConfig> })
        if (key === 'tracking_config') setTracking({ ...defaultTracking, ...obj as Partial<TrackingConfig> })
        if (key === 'whatsapp_business') setWhatsapp({ ...defaultWhatsApp, ...obj as Partial<WhatsAppBusinessConfig> })
        if (key === 'mcp_config') setMcp({ ...defaultMcp, ...obj as Partial<McpConfig> })
        if (key === 'ai_config') setAiConfig({ api_key: '', endpoint: 'https://api.openai.com/v1/chat/completions', model: 'gpt-4o-mini', ...obj as Partial<AiConfig> })
        if (key === 'social_media') setSocial({ ...defaultSocial, ...obj as Partial<SocialConfig> })
        if (key === 'telegram_bot') setTelegram({ ...defaultTelegram, ...obj as Partial<TelegramConfig> })
      })
    })
  }, [])
  ...
  const registerTelegramWebhook = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/telegram/setup-webhook`, {
        method: 'POST',
        ...
        <Tabs defaultValue="general" className="w-full">
          ...
          <TabsContent value="media" className="space-y-6 mt-6">
            <div className="bg-card rounded-lg border border-border p-6 space-y-4">
              <div className="flex items-center gap-3"><ImageIcon className="w-5 h-5 text-accent" /><h3 className="font-display font-semibold">Watermark Presets</h3></div>
            ...
            <div className="bg-card rounded-lg border border-border p-6 space-y-4">
              <div className="flex items-center gap-3"><ImageIcon className="w-5 h-5 text-accent" /><h3 className="font-display font-semibold">Media Trash (global)</h3></div>
              <MediaTrashPanel />
            </div>

            <div className="bg-card rounded-lg border border-border p-6 space-y-4">
              <div className="flex items-center gap-3"><ImageIcon className="w-5 h-5 text-accent" /><h3 className="font-display font-semibold">Audit Log</h3></div>
              <MediaAuditLogPanel />
            </div>
          </TabsContent>
          ...
          <TabsContent value="messaging" className="space-y-6 mt-6">
            ...
            <div className="bg-card rounded-lg border border-border p-6 space-y-4">
              <div className="flex items-center gap-3"><Send className="w-5 h-5 text-accent" /><h3 className="font-display font-semibold">Telegram Bot</h3></div>
              <p className="text-xs text-muted-foreground">
                Configure the Telegram listing bot. The bot token is set as <code>TELEGRAM_API_KEY</code> on the server and stays server-side.
              </p>
              <div className="grid sm:grid-cols-2 gap-4">
                <SettingsInput label="Bot Username" value={telegram.botUsername} onChange={(v) => setTelegram(p => ({ ...p, botUsername: v }))} placeholder="@DyarreListingsBot" />
                <SettingsInput label="Allowed Chat IDs (comma-separated)" value={telegram.allowedChatIds} onChange={(v) => setTelegram(p => ({ ...p, allowedChatIds: v }))} placeholder="123456789, 987654321" />
                <SettingsInput label="Default Listing Status" value={telegram.defaultStatus} onChange={(v) => setTelegram(p => ({ ...p, defaultStatus: v }))} placeholder="For Sale" />
                <SettingsInput label="Default Furnishing" value={telegram.defaultFurnishing} onChange={(v) => setTelegram(p => ({ ...p, defaultFurnishing: v }))} placeholder="Unfurnished" />
                <SettingsInput label="Default Completion" value={telegram.defaultCompletion} onChange={(v) => setTelegram(p => ({ ...p, defaultCompletion: v }))} placeholder="Ready" />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider block mb-1">Welcome / Help Message</label>
                <textarea
              ...
              <div className="space-y-2">
                ...
                <EndpointRow label="Telegram webhook" url={`${API}/api/telegram/webhook`} onCopy={copy} />
              </div>
              {webhookInfo && (
              ...
              <div className="text-xs text-muted-foreground p-3 bg-accent/10 rounded-md space-y-1">
                ...
                <p>1. Create a bot with <code>@BotFather</code> and set <code>TELEGRAM_API_KEY</code> on the server.</p>
                <p>2. Open a chat with your bot, send <code>/start</code>, get the <code>chat_id</code> and add it to <strong>Allowed Chat IDs</strong>.</p>
                <p>3. Click <strong>Register / Refresh Webhook</strong>. Telegram will then deliver messages to your bot.</p>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="automation" className="space-y-6 mt-6">
            <div className="bg-card rounded-lg border border-border p-6 space-y-4">
              ...
              <div className="space-y-2">
                <EndpointRow label="Create property" url={`${API}/api/bot/create-property`} onCopy={copy} />
                <EndpointRow label="Upload property image" url={`${API}/api/bot/upload-image`} onCopy={copy} />
                <EndpointRow label="List option values" url={`${API}/api/bot/list-options`} onCopy={copy} />
                <EndpointRow label="Telegram webhook" url={`${API}/api/telegram/webhook`} onCopy={copy} />
              </div>
              <div className="p-3 bg-secondary rounded-md text-xs space-y-1">
                ...
                <p className="text-muted-foreground">The bot key is stored as the <code>BOT_API_KEY</code> server environment variable. Rotate it any time by updating the env var and restarting PM2.</p>
              </div>
            ...
            <div className="bg-card rounded-lg border border-border p-6 space-y-4">
              ...
              <p className="text-xs text-muted-foreground">Optional override — use a custom OpenAI-compatible endpoint for visitor analysis.</p>
              <div className="grid sm:grid-cols-2 gap-4">
...
function LogosCard() {
  ...
  useEffect(() => {
    getSetting('branding_logos').then(v => {
      if (v && typeof v === 'object') setLogos(l => ({ ...l, ...v as object }))
    })
  }, [])
  ...
  const persist = async (next: typeof logos) => {
    setLogos(next)
    try {
      await saveSetting('branding_logos', next)
  ...
  const upload = async (slot: 'light' | 'dark', file: File) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/images/upload-branding`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: form,
      })
